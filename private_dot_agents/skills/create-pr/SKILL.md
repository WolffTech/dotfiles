...
3. Before querying GitHub or creating remote state, verify that `gh` is installed and authenticated. In proposal-only mode, continue from local evidence when GitHub access is unavailable and disclose that existing-PR and remote metadata were not verified. Do not install software or change authentication without user approval.
...
6. Resolve the base to an explicit remote-tracking ref. Fetch the selected target branch before comparison when network access is available. If fetching is unavailable, use the existing remote-tracking ref and disclose that its freshness was not verified.
...
11. Complete the selected mode:
    ...
    - Push `HEAD` only to the resolved head remote when necessary, setting upstream without force-pushing.
...
12. Read back the created pull request and verify its URL, title, target repository, base, head, and draft status before reporting success.
