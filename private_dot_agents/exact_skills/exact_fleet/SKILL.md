...
1. Locate the Fleet checkout. Use a path supplied by Nick first; otherwise check `~/Developer/fleet`, then `~/Developer/Fleet`, on the execution machine. Use the actual checkout path consistently. If both paths identify different directories, ask which checkout to use. This skill is managed by chezmoi and is separate from the Fleet checkout. Confirm the checkout contains `AGENTS.md`, `README.md`, `access.md`, and `machines/`. If it is missing or ambiguous, ask for the path; do not clone or configure anything automatically.
